...
기본 plan을 실행합니다. 기본 plan이 **없으면** 그 세 동사는 이전과 같이
`unknown flag "--"`로 거부합니다 — entry 이름을 받지 않으므로 끊을 이름이 없다는
TASK-207의 판정이 그대로 남아 있습니다. 그래서 `restart`에서 성립하는 **양방향**
동일성이 그 세 동사에서는 성립하지 않습니다. 여섯 fixture 중 넷에서 `up`/`down`/`stop`
12쌍이 갈립니다: plan이 없는 설정 셋(9쌍)에서는 맨 형태가 선언된 entry를 실제로
실행하고 (`dva down`은 전부 내리고 rc=0) `--` 형태만 거부하며, plan이 여럿이고 기본
plan이 없는 설정(3쌍)에서는 둘 다 거부하되 맨 형태는 "plan을 지정하라", `--` 형태는
`unknown flag "--"`라고 서로 다르게 말합니다. `build`는 반대 방향으로 한 쌍 더
갈립니다 (TASK-217). 모두 TASK-210 이전에도 같았던 측정값이고, 다시 판정할지는
TASK-216이 다룹니다.
