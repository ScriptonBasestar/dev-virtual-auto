...
plan이 여러 개고 `default_plan`이 없으면 맨 `dva restart`처럼 "plan을 지정하라"며
거부합니다. 예외는 `default_plan`이 있는 설정으로, 여기서는 맨 앞의 `--`가 아직
플래그로 취급되어 거부됩니다 (master에서도 동일한 기존 동작, TASK-210).
