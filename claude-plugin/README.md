...
| **dva-config** | `skills/dva-config/SKILL.md` | dva.yml 저작·감사·마이그레이션·진단 (ownership 모델, 변경 모드, 위험순 검증). 자동 트리거 |

`dva` skill은 Claude Code가 raw docker/compose/kubectl 대신 DVA를 사용하도록 강제하고,
`dva-config` skill은 dva.yml 설정 작업 시 자동으로 트리거됩니다 (`user-invocable: false`).
