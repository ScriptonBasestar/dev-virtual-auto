...
func TestSAMPlugin_Name(t *testing.T) {
	p := &SAMPlugin{}
	if p.Name() != "sam" {
		t.Errorf("expected 'sam', got %q", p.Name())
	}
}

func TestSAMPlugin_Up_NilConfig(t *testing.T) {
	p := &SAMPlugin{}
	pctx := &PluginContext{
		Entry:  &config.LifecycleEntry{SAM: nil},
		Logger: slog.Default(),
	}
...
func TestSAMPlugin_Down_NilConfig(t *testing.T) {
	p := &SAMPlugin{}
	pctx := &PluginContext{
		Entry:  &config.LifecycleEntry{SAM: nil},
		Logger: slog.Default(),
...
func TestSAMPlugin_Stop_NilConfig(t *testing.T) {
	p := &SAMPlugin{}
	pctx := &PluginContext{
		Entry:  &config.LifecycleEntry{SAM: nil},
		Logger: slog.Default(),
...
func TestSAMPlugin_Status_NilConfig(t *testing.T) {
	tmpDir := t.TempDir()

	p := &SAMPlugin{}
	pctx := &PluginContext{
		Entry:     &config.LifecycleEntry{Name: "sam-api", SAM: nil},
		ConfigDir: tmpDir,
		Logger:    slog.Default(),
	}
...
func TestSAMPlugin_DryRun_Up(t *testing.T) {
	p := &SAMPlugin{}
	env := config.NewEnvironment(nil, "/tmp", "/tmp")
	pctx := &PluginContext{
		Entry: &config.LifecycleEntry{
			Name: "sam-api",
			SAM: &config.SAMPluginConfig{
...
func TestSAMPlugin_Status_NoPidFile(t *testing.T) {
	tmpDir := t.TempDir()
