...
> **Auto-generated sections** — rule 9 (reserved commands) and rule 14 (section order)
> are regenerated from `internal/config` by `make generate` (tools/libgen) and bounded by
> `<!-- AUTOGEN:... -->` markers. Edit the Go source, not the marker block. Other rules are
> hand-authored. See `shared/library/README.md` for the ownership boundary.
...
9. **Reserved commands** — DVA built-in commands that MUST NOT appear as plain interaction
   ...
   Reserved (27, must not be interaction keys): `app`, `build`, `clean`, `completion`, `compose`, `config`, `console`, `doctor`, `down`, `help`, `infra`, `init`, `ktl`, `logs`, `ls`, `manifest`, `provision`, `restart`, `run`, `show`, `ssh`, `stack`, `status`, `stop`, `up`, `validate`, `version`
   Hookable (use `replace:` hooks, 7): `build`, `clean`, `down`, `logs`, `restart`, `stop`, `up`
...
14. **Section order (canonical)** — Canonical source:
    ...
    `version` → `vars` → `environment` → `env_file` → `stack` → `plans` → `default_plan` → `environments` → `sites` → `checks` → `applications` → `default_mode` → `suggestion_ignore` → `modes` → `health_checks` → `interaction` → `provision` → `modules` → `subprojects` → `endpoints` → `infra` → `ssh` → `devcontainer`
