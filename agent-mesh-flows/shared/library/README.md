...
points at. `replaceBlock` builds the marker string from the name it is handed and rewrites
that one pair; it never searches the repository for markers, so an
`<!-- AUTOGEN:NAME:start -->` pair the generator was not told about is silently left alone,
in this file or any other. `tools/libgen/main.go` is the authority for which regions exist;
the table is a lookup for *where the fact comes from*.
