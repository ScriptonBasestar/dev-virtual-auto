...
- [x] No live config warns on section order | verify: `/usr/bin/find ~/mydevbox -maxdepth 2 -name dva.yml -not -path '*/node_modules/*' | while read f; do (cd "$(dirname "$f")" && dva validate 2>&1 | /usr/bin/grep -F 'section order: found'); done` — expect no output over 31 configs
