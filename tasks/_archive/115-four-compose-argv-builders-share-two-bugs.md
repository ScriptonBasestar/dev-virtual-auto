...
- [x] The four copies are one | verify: `grep -rn 'SplitCommand' internal/cli/compose.go internal/runner/compose.go internal/lifecycle/compose.go | wc -l` — print the count and state the target
- [x] Path joining is uniform | verify: `grep -rn 'cfgDir + "/"' internal/ | wc -l` — must be 0
