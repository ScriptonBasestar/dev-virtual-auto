...
- [x] No tracked source names the old repo outside the guard's own fixtures | verify: `/usr/bin/find . -path ./.git -prune -o -path ./tmp -prune -o -path ./.omo -prune -o -path ./tasks -prune -o -type f -print0 | xargs -0 /usr/bin/grep -l 'ScriptonBasestar/dev-virtual-auto' | /usr/bin/grep -v corpus_urls_test.go ; test $? -ne 0` — **no output, exit 0 (TASK-199).** Both pipes were written `\|`. GFM processes a backslash escape inside a code span **only in a table row**, and this is a list item, so the backslash was literal: as published the command handed `|` to `find` as an argument and died with `find: |: unknown primary or operator` before reaching `xargs`. The logic was sound — `grep -v` exits 1 when nothing survives, which `test $? -ne 0` inverts into a pass — but it never ran. `tools/doccheck/verifyrun.go:85-91` already states this rule and applies it only to `go test … -run` bindings
