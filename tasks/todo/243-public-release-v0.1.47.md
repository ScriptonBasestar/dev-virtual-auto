...
- [ ] Create a local lightweight `v0.1.47` tag at `release_commit`; use a clean detached worktree
  at that exact identity | verify: `test "$(git cat-file -t v0.1.47)" = commit && test "$(git rev-list -n1 v0.1.47)" = "$RELEASE_COMMIT"`
- [ ] Record the reviewed `release-notes/v0.1.47.md` SHA-256 and pass `make release-preflight`
...
Fill this section only after the final postflight succeeds. Record the release commit, notes
SHA-256, release URL, asset identities, command exit results, and cleanup result. Do not paste
tokens, Keychain command output, request headers, or shell environments.
