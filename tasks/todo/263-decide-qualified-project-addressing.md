...
- [ ] Decide explicit import/export versus automatic registration; the recommended default is explicit import with no flattening or automatic reachability, and `/` denotes only an explicitly imported parent-visible item | verify: human — any broader exposure requires a bounded namespace and compatibility proof
