...
- [ ] Fake-sops fault tests and a pinned real-sops integration test both pass on every supported OS; safe-writer and command tests run in a Linux/macOS/Windows CI matrix for declared support, while platforms outside that matrix have an explicit unsupported fail-closed test | verify: `/usr/bin/grep -Eq '^func TestConfigEnvRealSOPS\(' internal/integration/config_env_test.go && /usr/bin/grep -q 'config-env-platform' .github/workflows/ci.yml && make test-integration`
